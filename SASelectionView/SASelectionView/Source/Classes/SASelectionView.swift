...
open class SASelectionView: BaseView {
    ...
    public class func show(title: String?, sections: [SectionItem] = [], showSearchBar: Bool = false, emptySearchRowTitle: String? = nil, dismisshandler: (() -> ())? = nil, emptyRowHandler: ((_ searchText: String) -> ())? = nil, selectedOption: @escaping (( _ section : Int, _ row: Int, _ value: String) -> ()))
    {
        if let keyWindow = UIApplication.shared.keyWindow {
            let nibViews = UINib(nibName: "SASelectionView", bundle: Bundle.frameworkBundle).instantiate(withOwner: nil, options: nil)
            if let nibView: SASelectionView = nibViews.first as? SASelectionView {
                ...
                fatalError("SASelectionView: Nib not instantiated")
            }
        }
    }
        ...
        DispatchQueue.main.async {
            if let keyWindow = UIApplication.shared.keyWindow {
                ...
                for subview in subviews {
                    if subview.isKind(of: SASelectionView.self) {
...
extension SASelectionView: UITextFieldDelegate {
